...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        self.linear = nn.Linear(embed_dim+(channel_ratio*256),2)
    ...
    def forward(self, x,x_dct):
        ...
        x_base_dct = self.maxpool(self.act1(self.bn1(self.conv1(x_dct))))
        # 1 stage
        x = self.conv_1(x_base_dct, return_x_2=False)
        print(x_base.shape,self.trans_patch_conv(x_base).shape)
        ...
        out = torch.cat([x_p,x_t[:,0]],dim=-1)
        out = self.linear(out)
        return out

m = Conformer()
m(torch.rand(1,3,224,224),torch.rand(1,3,224,224))